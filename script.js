...
window.addEventListener('mousemove', (e) => {
    if (isInteractiveTarget(e.clientX, e.clientY)) { mouse.x = -2000; mouse.y = -2000; return; }
    mouse.x = e.clientX; mouse.y = e.clientY;
});
window.addEventListener('touchmove', (e) => {
    if(e.touches.length > 0) {
        const t = e.touches[0];
        if (isInteractiveTarget(t.clientX, t.clientY)) { mouse.x = -2000; mouse.y = -2000; return; }
...
window.addEventListener('touchstart', (e) => {
    if(e.touches.length > 0) {
        const t = e.touches[0];
        if (isInteractiveTarget(t.clientX, t.clientY)) { mouse.x = -2000; mouse.y = -2000; }
    }
}, { passive: true });
...
searchInput.addEventListener('focus', () => { const r = searchInput.getBoundingClientRect(); gravityWell.x = r.left + r.width/2; gravityWell.y = r.top + r.height/2; gravityWell.active = true; });
searchInput.addEventListener('blur', () => { gravityWell.active = false; });
...
themeToggle.addEventListener('click', () => {
    ...
    if (document.documentElement.getAttribute('data-theme') === 'dark') {
        document.documentElement.removeAttribute('data-theme'); themeToggle.innerHTML = '<i class="fas fa-moon"></i>';
        localStorage.setItem('iapps_theme', 'light');
    } else {
        document.documentElement.setAttribute('data-theme', 'dark'); themeToggle.innerHTML = '<i class="fas fa-sun"></i>';
